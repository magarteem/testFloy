...
export const SelectToolsElementMui = ({
  ...
  const [personName, setPersonName] = useState<string[]>(value.map((x: any) => x.label));
            ...
            <Stack direction="row" spacing={1} sx={styleSxTool.stack}>
              {selected.map((values: any) => {
                let iconTools = options.map((op: any) => {
                  const temp = op.options.find((x: any) => x.label === values);
                ...
                return (
                  <Chip
                    ...
                    key={values}
                    label={values}
                    avatar={<img alt="values" src={iconTools.filter((y) => y !== undefined)[0].imgIcons} />}
                    sx={styleSxTool.chip}
                  />
                );
              })}
        ...
        {options.map((x: any, index: any) => {
          ...
          return x.options.map((p: any) => {
            if (t === 0) {
              t++;
              return (
                <div className={s.openMenu} onClick={() => setClassesHiddenCount(index)}>
                  <img
                    className={cn({
                  ...
                  />
                  <ListItemText sx={styleSxTool.listItem} primary={x.label} />
                </div>
              );
            } else {
              let iconTools = options.map((op: any) => {
                const temp = op.options.find((x: any) => x.label === p.label);
                return temp;
              });
              ...
              return (
                <MenuItem
                  ...
                  value={p.label}
                >
                  <Checkbox style={{ width: "15px", height: "15px" }} sx={styleSxTool.checkbox} checked={personName.indexOf(p.label) > -1} />
                  <img alt="values" src={iconTools.filter((y) => y !== undefined)[0].imgIcons} />
