...
export const OutgoingNotificationPageOne = () => {
  const data = useAppSelector((state) => state.notificationSliceReducer);

  const { id_inComingNotification } = useParams();
  const dataOneNews = data.adsList?.find((x) => `${x.id}` === id_inComingNotification);

  if (!dataOneNews) return <h1>Loading ...</h1>;

  const looking = ["Музыкант", "Коллектив"].indexOf(dataOneNews.required.label);
        ...
        <StylesFullScreen>
          <RibbonLayout>
            <div className={s.incomingNotificationPageOne}>
              <HeaderStylesWrapper cancelImgIcon={arrow_back} textLabel="Запросы" anyIconsFirst={share} tsxElement={<LongMenu />} />
  ...
  ${dataOneNews.required.label.toLowerCase()}`}
                    ...
                    {dataOneNews.payment ? <p>{dataOneNews.payment}</p> : <p>Не коммерческое</p>}

                    {dataOneNews.workingConditions && <p>{dataOneNews.workingConditions.label}</p>}
                  </div>
                </div>
                ...
                <div className={s.styleAbout}>
                  ...
                  {` ${calculateAge(dataOneNews.toAge)}-${calculateAge(dataOneNews.fromAge)}`}
                </div>

                {dataOneNews.work_experience && (
                  <div className={s.styleAbout}>
                    <span className={s.titleSpan}>Опыт работы/выступлений:</span>
                    {typeof dataOneNews.work_experience == "string" && <span>{dataOneNews.work_experience}</span>}
                  </div>
                )}
                ...
                <div className={cn(s.reStyleImportant, s.border)}>
                  <SkillsLayoutTools skillsDataItem={dataOneNews.tool} skillsCategoryTitle="Инструменты" />
                </div>

                <div className={s.reStyleImportant}>
                  <SkillsLayoutGenre skillsDataItem={dataOneNews.genre} skillsCategoryTitle="Жанр" />
                </div>
         ...
         />
                ...
                <div className={s.pending}>
                  {dataOneNews.waitingForResponse.status === 0 && <Pending />}
                  {dataOneNews.waitingForResponse.status === 1 && <Received status={dataOneNews.waitingForResponse} />}
                  {dataOneNews.waitingForResponse.status === 2 && <Rejected />}
                </div>
              </section>
            </div>
          </RibbonLayout>
